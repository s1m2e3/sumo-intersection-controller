...
from model import HybridModel, V_TURN_LOW, V_TURN_HIGH, D_APPROACH, D_EGO, D_SUM, D_RIVAL, K_MAX, ARM_LENGTH as _ARM_LENGTH
...
def _write_cfg(routes_name):
    p = SUMO_DIR / "hybrid.sumocfg"
    p.write_text(f"""<?xml version="1.0" ?>
<configuration>
  <input>
...
def run(gui=False, ew_vph=FLOW_VPH, ns_vph=FLOW_VPH, model_path=None):
    ...
    print(f"\n{'='*88}")
    print(f"  HybridModel | 4-way | {SIM_SECONDS:.0f}s | EW={ew_vph} NS={ns_vph} vph (+20% turns)")
    print(f"{'='*88}")
    ...
    for step in range(control_steps):
        ...
        if all_tracked:
            _, mu_cf_t, sum_t, rival_list = compute_social_force_2d(all_tracked, snap)
        ...
        vids_batch  = []
        u_wp_list   = []; mu_wp_list = []; mu_cf_list = []
        ...
        for vid in all_vids:
            ...
            # mu_wp
            dv      = max(v - v_lead, 1e-3)
            ttc     = max(gap, 0.0) / dv
            mu_wp_v = _mu_wp(ttc, d_jct, road)
            ...
            nn_weights_step.append((1.0 - mu_wp_v) * (1.0 - mu_cf_v))
        ...
        mu_wp_t  = torch.tensor(mu_wp_list,        dtype=torch.float32)  # [N]
        mu_cf_t  = torch.tensor(mu_cf_list,        dtype=torch.float32)  # [N]

        with torch.no_grad():
            a_out = model(x_seq_t, sum_t, rival_t, u_wp_t, mu_wp_t, mu_cf_t)  # [N]
